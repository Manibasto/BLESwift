...
//    var dataBuffer:NSMutableData!
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        print("Discovered \(peripheral.name ?? "No peripheral Device found") at \(RSSI)")
...
//        dataBuffer.length = 0
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        print("Failed to connect to \(peripheral) (\(error?.localizedDescription ?? "Error"))")
        connectionIndicatorView.layer.backgroundColor = UIColor.red.cgColor
        self.disconnect()
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        if error != nil {
            print("Error discovering services: \(error?.localizedDescription ?? "Error")")
            disconnect()
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        if error != nil {
            print("Error discovering characteristics: \(error?.localizedDescription ?? "Error")")
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if error != nil {
            print("Error updating value for characteristic: \(characteristic) - \(error?.localizedDescription ?? "Error")")
            return
        }
        ...
        // make sure we have a characteristic value
        let nextChunk = String(data: value, encoding: String.Encoding.utf8)
        ...
        if (nextChunk == Device.EOM) {
            ...
            let data = NSData(data: dataBuffer as Data)

        
            print("Final message:")
            
            self.imageView1.image = UIImage.init(data: data as Data)
//            dataBuffer.length = 0
            dataBuffer.count = 0
...
//            }
        ...
        print("DataBuffer--->",dataBuffer.count)
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?) {
        // if there was an error then print it and bail out
        if error != nil {
            print("Error changing notification state: \(error?.localizedDescription ?? "Error")")
            return
        }
